...
let lista  = [];
// let Acuaticos = ["Tiburon","ballena"], Terrestres = ["Vaca","Conejo"], Aereos = ["Aguila","Colibri"];
// lista[0] = Acuaticos; lista[1] = Terrestres; lista[2] = Aereos;
...
let usuario = parseInt(prompt("Usuario cuales de las siguientes listas desea ver:\n1. Acuaticos\n2. Terrestres\n3. Aereos"));
let numPaginar = 2;
...
if(lista[usuario-1].length%numPaginar==0){
    // let paginacion = promt(`Total de datos en la lista: ${lista[usuario-1].length}`);
    // let menu = "";
    for(let i = 0; i<lista[usuario-1].length;i+=numPaginar){
        menu += `${cont} - ${i+numPaginar} hay ${lista[usuario-1].length}\n`;
    ...
    for (let i = 0; i<lista[usuario-1].length;i+=numPaginar){
        if(lista[usuario-1].length==i+1){
            menu +=`${cont} hay ${lista[usuario-1].length}\n`;
